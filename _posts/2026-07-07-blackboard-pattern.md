...
这些问题有一个共同的解法：<mark>**不让步骤之间直接通信**，而是放一块所有人都能读写的「黑板」</mark>。每个步骤只管往黑板上写结果、从黑板上读输入。这就是黑板模式。
...
3. <mark>**黑板必须线程安全**</mark>。 多个 Worker 可能同时读写黑板，如果没有锁保护，就会出现数据竞争。最基础的实现是加读写锁；更精细的做法是对不同数据分区加独立的锁。
...
<mark>**key 协议的脆弱性**是最大痛点</mark>。Worker 之间通过约定 key 交换数据，key 名拼错、类型搞错、忘了写某个 key——**编译期都不会报错，只会在运行时表现为"读不到数据"**。应对方法是在框架层做 key 注册和类型校验，或者尽量用结构化抽屉代替交换区。
